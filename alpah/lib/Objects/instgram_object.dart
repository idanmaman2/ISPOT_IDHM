...
class InstaObject implements IInsta {
  ...
  static Future<InstaObject> fromInstaUserName(String profileName) async {
    ...
    do {
      insta = await http
          .read(Uri.parse("$_siteHeader$profileName$_dataScrapeTail"));
      await Future.delayed(const Duration(milliseconds: 40));
    } while (insta.contains(_errorText));
    var instaJson = jsonDecode(insta);
    links.addAll((instaJson['graphql']['user']['edge_owner_to_timeline_media']
            ['edges'])
        .map((node) => node['node']['display_url']));
    return InstaObject(
        links,
        profileName,
        instaJson['graphql']['user']['biography'],
        instaJson['graphql']['user']['profile_pic_url_hd'],
        int.parse(instaJson['graphql']['user']["edge_followed_by"]["count"]));
  }
  ...
  static Future<String> findInstaName(String fullName) async {
    ...
    do {
      insatSearch =
          await http.read(Uri.parse("$_siteHeader$_searchAdress$fullName"));
      print(insatSearch);
